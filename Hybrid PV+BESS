...
def optimize_dispatch_dp(inputs: SimulationInputs) -> Dict[str, np.ndarray]:
    ...
    # Budget annuel de décharge autorisé (définition cohérente avec equivalent_cycles)
    max_total_discharge = float(inputs.max_cycles_per_year * inputs.batt_energy_mwh)
    ...
    def nearest_cycle_index(value: float) -> int:
        value = min(max(value, 0.0), max_total_discharge)
        return int(np.argmin(np.abs(cycle_grid - value)))
    ...
    # value_next[soc_idx, cycle_idx]
    value_next = np.full((soc_steps, cycle_steps), neg_inf, dtype=float)
    value_next[final_soc_idx, :] = 0.0  # toute quantité de budget restant est acceptable en fin d'année

    # Policies
    policy_next_soc = np.full(
        (T, soc_steps, cycle_steps),
        -1,
        dtype=np.int16 if max(soc_steps, cycle_steps) < 32000 else np.int32
    )
    policy_next_cycle = np.full(
        (T, soc_steps, cycle_steps),
        -1,
        dtype=np.int16 if max(soc_steps, cycle_steps) < 32000 else np.int32
    )
    ...
    for t in range(T - 1, -1, -1):
        value_now = np.full((soc_steps, cycle_steps), neg_inf, dtype=float)
        ...
        for i in range(soc_steps):
            ...
            for c in range(cycle_steps):
                used_discharge_budget = cycle_grid[c]
                ...
                for j in transitions[i]:
                    ...
                    # quantité de budget cycle consommée à cette heure
                    cycle_use = 0.0
                    ...
                    if delta_soc > 1e-12:
                        ...
                        # filtre charge réseau
                        if grid_charge > 1e-9 and grid_buy_t > charge_threshold:
                            continue

                        # heuristique anti-arbitrage conservée
                        MIN_SPREAD = 10.0
                        ...
                        cycle_use = discharge_candidate  # budget compté en MWh déchargés

                        # filtre décharge
                        if discharge_candidate > 1e-9 and batt_sell_t < discharge_threshold:
                            continue

                        # contrainte cycle: ne pas dépasser le budget annuel
                        if used_discharge_budget + cycle_use > max_total_discharge + 1e-9:
                            continue

                    # contrainte export réseau
                    ...
                    if total_export > inputs.grid_export_limit_mw:
                        ...
                        curtailed_pv += reduction_pv
                        excess -= reduction_pv

                        if excess > 0:
                            discharge_candidate = max(discharge_candidate - excess, 0.0)

                        # si la décharge a été réduite par la limite réseau,
                        # la consommation réelle du budget cycle doit être corrigée
                        if delta_soc < -1e-12:
                            cycle_use = discharge_candidate

                    next_cycle_used = used_discharge_budget + cycle_use
                    c_next = nearest_cycle_index(next_cycle_used)

                    # coût de cycle appliqué systématiquement sur la décharge réelle
                    ...
                    total_val = reward + value_next[j, c_next]

                    if total_val > best_val:
                        best_val = total_val
                        best_j = int(j)
                        best_c_next = int(c_next)

                value_now[i, c] = best_val
                policy_next_soc[t, i, c] = best_j
                policy_next_cycle[t, i, c] = best_c_next
    ...
    state_cycle = nearest_cycle_index(0.0)
    ...
    cycle_cost = np.zeros(T, dtype=float)
    cycle_budget_used = np.zeros(T + 1, dtype=float)

    for t in range(T):
        next_soc = int(policy_next_soc[t, state_soc, state_cycle])
        next_cycle = int(policy_next_cycle[t, state_soc, state_cycle])

        if next_soc < 0 or next_cycle < 0:
            raise RuntimeError(
                f"Policy failure at t={t}, soc_state={state_soc}, cycle_state={state_cycle}"
            )
        ...
        cycle_cost[t] = discharge[t] * inputs.cycle_cost_eur_per_mwh

        cycle_budget_used[t + 1] = cycle_budget_used[t] + discharge[t]

        state_soc = next_soc
        state_cycle = next_cycle
    ...
    total_cycle_cost = float(cycle_cost.sum())
    ...
    result = {
        ...
        "cycle_budget_used_mwh": cycle_budget_used,
        ...
        "total_cycle_cost": np.array([total_cycle_cost]),
...
def build_summary_table(result: Dict[str, np.ndarray], pv_stats: Dict[str, float], inputs: SimulationInputs) -> pd.DataFrame:
    rows = [
        ...
        ("Coût de cyclage batterie", float(result["total_cycle_cost"][0]), "EUR"),
        ...
        ("Budget cycle utilisé", float(result["cycle_budget_used_mwh"][-1]), "MWh"),
        ("Budget cycle max", float(inputs.max_cycles_per_year * inputs.batt_energy_mwh), "MWh"),
